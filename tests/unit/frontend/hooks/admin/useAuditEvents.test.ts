...
describe("useAuditEvents", () => {
  ...
  it("loads and appends cursor pages", async () => {
    ...
    expect(api.listAuditEvents).toHaveBeenLastCalledWith({ beforeId: 8, action: undefined, success: undefined });
  });
  ...
  it("reloads when result filter changes", async () => {
    ...
    await waitFor(() => expect(api.listAuditEvents).toHaveBeenLastCalledWith({ beforeId: undefined, action: undefined, success: false }));
  });
});
